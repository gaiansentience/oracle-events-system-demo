...
begin
    ...
    l_json := events_json_api.get_customer_event_purchases_by_email(p_customer_email => l_customer_email, p_event_id => l_event_id, p_formatted => true);   
    dbms_output.put_line(events_json_api.json_as_clob(l_json));

    dbms_output.put_line('customer did not purchase tickets:');
    l_json := events_json_api.get_customer_event_purchases_by_email(p_customer_email => l_nonpurchase_customer, p_event_id => l_event_id, p_formatted => true);   
    dbms_output.put_line(events_json_api.json_as_clob(l_json));
