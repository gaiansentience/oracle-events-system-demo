...
v_venue_id := venue_api.get_venue_id(p_venue_name => 'Another Roadside Attraction');
...
v_customer_id := customer_api.get_customer_id('Andi.Warenko@example.customer.com');
events_test_data_api.delete_customer_data(v_customer_id);

--delete test reseller created
v_reseller_id := reseller_api.get_reseller_id(p_reseller_name => 'Ticket Factory');
